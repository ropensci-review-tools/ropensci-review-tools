...
`num_cores`     |     Use specified number of cores for parallel processing, defaulting to one half of all available cores. If set to `1L` , calculate as single job, displaying progress information on screen. See note.
...
Parallel processing may be unreliable, as each job requires writing and
 ...
 processing by setting `num_cores = 1L` .
