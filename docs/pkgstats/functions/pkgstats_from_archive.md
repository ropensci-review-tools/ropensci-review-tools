...
* `save_full`: If `TRUE`, full [pkgstats](pkgstats) results are saved for each
package to files in `results_path`.
* `save_ex_calls`: If `TRUE`, the results of the `external_calls` component
are saved for each package to files in `results_path` (only if `save_full = FALSE`).
